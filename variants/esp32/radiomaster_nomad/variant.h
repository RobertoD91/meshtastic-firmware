 ...
 * - the DIO5-DIO8 RF switch table is taken from the ExpressLRS LR1121 driver
 */
...
/*
  No TCXO reference voltage is configured: the LR1121 runs from its own
  crystal (XTAL mode) and DIO3 is left free. The ExpressLRS target does not
  document a DIO3 TCXO voltage; if the module turns out to need one, define
  LR11X0_DIO3_TCXO_VOLTAGE here instead.
*/
...
/*
  LR1121 DIO5-DIO8 drive the module's antenna / T-R switch. The table in
  rfswitch.h matches the ExpressLRS LR1121 driver configuration for this
  module (verified against ExpressLRS LR1121Driver::SetDioAsRfSwitch).
*/
...
/*
  Built-in fan, controlled by GPIO2. earlyInitVariant() starts it at the
  very beginning of boot (before radio detection) so it spins even if the
  LR1121 is never found. It can still be turned off through the
  pa_fan_disabled LoRa config option.
*/
